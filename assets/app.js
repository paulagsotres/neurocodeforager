...
async function loadEntries(){
  ...
  ENTRIES_CACHE.forEach(e => {
    e._author_tokens = e.authors
      ? e.authors.split(/[,;]+/).map(a => a.trim().split(/\s+/)[0]).filter(Boolean).join(' ')
      : '';
  });
...
async function initHome(){
  ...
  const fuseGeneral = new Fuse(entries, {
    includeScore: true,
    threshold: 0.3,
    ignoreLocation: true,
    ...
    keys: [
      ...
      { name: 'data_acquisition', weight: 0.08 },
      { name: 'animal',           weight: 0.05 },
      { name: 'language',         weight: 0.04 },
    ],
  });

  const fuseAuthors = new Fuse(entries, {
    includeScore: true,
    threshold: 0.0,
    distance: 0,
    ignoreLocation: true,
    minMatchCharLength: 3,
    keys: [{ name: '_author_tokens', weight: 1 }],
  });
  ...
  function runSearch(query){
    ...
    const authorHits  = fuseAuthors.search(query).map(r => r.item);
    const authorIds   = new Set(authorHits.map(e => e.id));
    const generalHits = fuseGeneral.search(query).map(r => r.item).filter(e => !authorIds.has(e.id));
    const hits = [...authorHits, ...generalHits];
